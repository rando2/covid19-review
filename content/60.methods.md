...
As with all scientific publishing, the strengths and weaknesses of each analysis affects how it should be interpreted; however, the velocity of publication (termed an “infodemic”) presents significant challenges for any effort to stay up to date with current findings.
...
This problem therefore represented a strong candidate for a massive online open publishing approach to collaboration.
Manubot provided an open publishing backend for this project.
...
The manuscript content could then be rendered from markdown into several different formats, including pdf, HTML, and docx.

However, this particular project presented unique challenges that required expansion of several functionalities in Manubot.
The first category of expansions related to the technical barrier to entry, as most contributors were from biomedical backgrounds and had limited experience with git.
We developed training resources to allow for participation through GitHub's web interface, added support for the citation of clinical trial identifiers, and added a spell-checking feature to Manubot to make its feedback more similar to what would be provided by a What You See Is What You Get editor such as Google Docs.
We also expanded the range of formats and options available for outputting a Manubot project.
The other category of expansions was necessary due to the extreme volume and velocity of COVID-19 publication.
Manubot generates figures from code, but for this project, we adapted this workflow to allow for the retrieval of up-to-date data from online sources.
This approach allowed us to automatically update information such as the number of clinical trials investigating specific therapeutics and worldwide trends in clinical trial recruitment.
...
This project illustrates that Manubot is an adaptable workflow that can handle even an extreme volume and velocity of information, and that its back-end technical complexity is not prohibitive to the inclusion of researchers from non-technical backgrounds.
